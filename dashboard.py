...
# Auto-refresh control (Meta Tag implementation)
...
refresh_interval_minutes = st.sidebar.number_input(
    ...
    help="Set to 0 to disable. Refreshes the entire page to get new data."
)
if refresh_interval_minutes > 0:
    st.markdown(f'<meta http-equiv="refresh" content="{refresh_interval_minutes * 60}">', unsafe_allow_html=True)
...
if not filtered_df.empty:
    ...
    fig_price.update_layout(title=f'<b>Price and Volume for {selected_ticker} ({chart_type} Chart)</b>', template="plotly_dark", yaxis=dict(title='Price (USD)', gridcolor='rgba(255,255,255,0.1)'), yaxis2=dict(title='Volume', overlaying='y', side='right', showgrid=False), legend=dict(x=0.01, y=0.99), xaxis_rangeslider_visible=False)
    ...
    with col1:
        ...
        fig_sentiment.update_layout(title=f'<b>Daily Average Sentiment Scores</b>', template="plotly_dark", yaxis=dict(title='Average Sentiment Score', gridcolor='rgba(255,255,255,0.1)'), legend=dict(x=0.01, y=0.99))
    ...
    with col2:
        ...
        fig_articles.update_layout(title=f'<b>Daily News Article Count</b>', template="plotly_dark", yaxis=dict(title='Number of Articles', gridcolor='rgba(255,255,255,0.1)'))
